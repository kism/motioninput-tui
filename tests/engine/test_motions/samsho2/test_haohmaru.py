...
# the two sides, which is most of what the roster has.


def test_forward_and_a_button_is_the_throw(play) -> None:
    assert play([press(FORWARD, 0), press(NEO_B, 60)]).moves[0] == "Yokonage"
...
def test_back_and_the_same_button_is_the_same_throw(play) -> None:
    ...
    which move you get."""
    assert play([press(BACK, 0), press(NEO_B, 60)]).moves[0] == "Yokonage"
...
def test_down_and_the_button_is_not_a_throw(play) -> None:
    ...
    assert play([press(DOWN, 0), press(NEO_B, 60)]).moves == []


def test_the_heavy_slash_throw_is_a_different_move(play) -> None:
    assert play([press(FORWARD, 0), press(NEO_D, 60)]).moves[0] == "Shinkuu Tomoe Nage"
